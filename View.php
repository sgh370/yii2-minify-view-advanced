...
class View extends \yii\web\View
{
    ...
    public function init()
    {
        ...
        if (isset($this->excludeModules[$module]))
        {
            if (empty($this->excludeModules[$module]) || isset($this->excludeModules[$module][$controller]))
            {
                $this->enableMinify = false;
            }
        }

        $path = ltrim(trim(Yii::$app->request->url), '/');
        if (!empty($this->excludeUrls))
        {
            foreach ($this->excludeUrls as $url)
            {
                if (mb_strpos($path, $url) === 0)
                {
                    $this->enableMinify = false;
                    break;
                }
